...
graph TB
    subgraph "houki-hub family"
        direction TB
        Lib["📦 npm Library<br/>（型定義・APIクライアント）"]
        MCP["🔌 MCP Server<br/>（AIエージェント連携）"]
        Skill["🧩 Claude Skill<br/>（実務ワークフロー）"]
        Docs["📖 Doc Site<br/>（仕様・利用ガイド）"]
    end

    Source[("外部API / 公式データソース<br/>e-Gov / 国税庁 / 厚労省 等")] --> Lib
    Lib --> MCP
    Lib --> Skill
